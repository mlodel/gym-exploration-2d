...
def constant_velocity_prediction_path(initial_pos, initial_vel, n_prediction_steps, dt=0.1):
  """
  Compute a constant velocity prediction path for a given number of prediction steps.
  The future positions are computed under the assumption that the agent does not change it's velocity.
  """
  pos_vec = np.zeros([n_prediction_steps, 2])
  pos_vec[0, :] = initial_pos + dt * initial_vel
  for ii in range(1, n_prediction_steps):
    pos_vec[ii, :] = pos_vec[ii-1, :] + dt * initial_vel
...
def create_map_from_png(file_name,resolution,map_size,map_center,data_path='../data',):
  ...
  if obst_idx.size > 0:
    ...
    grid_map['Closest X'] = obst_idx
    grid_map['Closest Y'] = obst_idx
