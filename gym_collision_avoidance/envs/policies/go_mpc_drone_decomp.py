...
from gym_collision_avoidance.envs.maps.constraint_generator import ConstraintGen
from gym_collision_avoidance.envs.utils.block_stdout import stdout_redirected
import contextlib


class GoMPCDroneDecomp(Policy):
    def __init__(self):
        Policy.__init__(self, str="GoMPCDroneDecomp")
        ...
        self.acceleration_weigth_ = 0.01
        self.angular_velocity_weight_ = 0.001  # 0.001
        self.angular_acc_weight_ = 0.01
    ...
    def run_solver(self, ego_agent, other_agents, update):
        ...
        for N_iter in range(0, self.FORCES_N):
            ...
            for c in range(self.M):
                ...
                d = 3 * c
                self.FORCES_all_parameters[k + 70 + d] = self.linear_constraints["a"][
                    c
                ][0]
                self.FORCES_all_parameters[k + 71 + d] = self.linear_constraints["a"][
                    c
                ][1]
                self.FORCES_all_parameters[k + 72 + d] = self.linear_constraints["b"][c]
