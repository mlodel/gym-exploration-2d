...
try
FORCEScleanup('StaticMPCsolver_drone','all');
catch
end
...
lb_R = [ -2  -2, 0, -50,   -50,    -1.5*pi, -2, -2.0];
%lb_R = [ 0, -2, 0, -50,   -50,    -1.5*pi];
model.lb = lb_R;

% Upper limits for robot
ub_R = [ +2,  +2, 10000, +50,   +50,    +1.5*pi, 2.0, 2.0];
...
model.eq = @(z, p) dynamic_scenario_drone(z(4: 8),z(1: 3), p, dt);
...
codeoptions = getOptions('StaticMPCsolver_drone');
