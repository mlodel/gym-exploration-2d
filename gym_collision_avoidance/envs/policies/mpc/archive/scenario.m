...
lb_R = [ 0,  -2, -50,   -50,    -1.5*pi];
model.lb = lb_R;

% Upper limits for robot
ub_R = [ +1,  +2, +50,   +50,    +1.5*pi];

model.ub =ub_R;
%%
for i=1:model.N
    %% Objective function
    model.objective{i} = @(z, p) first_order_objective_scenario(z(3: 5),z(1: 2), p,i); 

    model.ineq{i} = @(z,p) inequality_constr_scenario(z(3: 5),z(1: 2), p, i);
...
model.eq = @(z, p) first_order_dynamics(z(3: 5),z(1: 2), p,dt);
...
model.xinitidx = 3:5; % use this to specify on which variables initial conditions are imposed
