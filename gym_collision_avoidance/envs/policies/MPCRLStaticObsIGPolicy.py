...
class MPCRLStaticObsIGPolicy(Policy):
    ...
    def network_output_to_action(self, id, agents, network_output, new_goal=True):
        ...
        if new_goal:
            if np.linalg.norm(
                    agent.pos_global_frame - agent.goal_global_frame) > self.reference_velocity_ * self.FORCES_N * self.dt:
        ...
        if exit_flag == 1:
            ...
            if self.infeas_count < Config.REPEAT_STEPS and not new_goal:
                return np.array([self.FORCES_x0[self.infeas_count*self.FORCES_TOTAL_V],
    ...
    def mpc_output(self, id, agents):
        ...
        if exit_flag != 1:
            exit_flag, output = self.run_solver(agent,other_agents, update=False)
        if exit_flag == 1:
            terminal_pose = np.array([output["x"+str(self.FORCES_N)][self.FORCES_NU],output["x"+str(self.FORCES_N)][self.FORCES_NU+1]])
