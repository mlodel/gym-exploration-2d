...
def IG_single_agent(
    ...
    obstacle_1 = [(10, 10), (1, 10), (1, 1), (10, 1)]
    obstacle.extend([obstacle_1])
...
def test_scenario(
    ...
    agents.append(
        Agent(10, 0, 0, 0, 0.2, pref_speed, 0, StaticPolicy, FirstOrderDynamics, [], 1)
    )
    agents.append(
        Agent(
            -6, -12, 0, 0, 0.2, pref_speed, 0, StaticPolicy, FirstOrderDynamics, [], 2
        )
    )
    agents.append(
        Agent(11, 0, 0, 0, 0.2, pref_speed, 0, StaticPolicy, FirstOrderDynamics, [], 3)
    )
    agents.append(
        Agent(
            -5, -12, 0, 0, 0.2, pref_speed, 0, StaticPolicy, FirstOrderDynamics, [], 4
        )
    )
    agents.append(
        Agent(12, 0, 0, 0, 0.2, pref_speed, 0, StaticPolicy, FirstOrderDynamics, [], 5)
    )
    agents.append(
        Agent(
            -4, -12, 0, 0, 0.2, pref_speed, 0, StaticPolicy, FirstOrderDynamics, [], 6
        )
    )

    if "MPCRLStaticObsPolicy" == str(agents[0].policy) or "MPCStaticObsPolicy" == str(
        agents[0].policy
    ):
        agents[0].policy.static_obstacles_manager.obstacle = obstacle
