...
def plot_episode(agents, obstacles, in_evaluate_mode,
    ...
    if env_map:
        ax.imshow(env_map.static_map,
                  extent=[-env_map.x_width / 2., env_map.x_width / 2., -env_map.y_width / 2., env_map.y_width / 2.],
                  cmap=plt.cm.binary)
    ...
    if agents[0].ig_model is not None:
        ...
        # ax3.imshow(agents[0].ig_model.targetMap.ego_map, vmin=0, vmax=1, cmap='jet', origin='upper')
        ax3.imshow(agents[0].ig_model.targetMap.bin_ego_map, cmap=plt.cm.binary, origin='upper')
        ax3.set_yticklabels([])
        ax3.set_xticklabels([])
...
def draw_agents(agents, obstacle, circles_along_traj, ax, ax2=None, last_index=-1):
    ...
    for i, agent in reversed(list(enumerate(agents))):
        ...
        if circles_along_traj:
                        ...
                        for ind in range(agent.policy.FORCES_N):
                            ...
                            for mix_id in range(n_mixtures):
                                ...
                                ax.add_patch(Ellipse(agent.policy.all_predicted_trajectory[id, mix_id, ind, :2],
                                                     width=2 * (agent.radius + agent.policy.all_predicted_trajectory[
                                                         id, mix_id, ind, 2]),
                                                     height=2 * (agent.radius + agent.policy.all_predicted_trajectory[
                                                         id, mix_id, ind, 3]), fc=c, ec=other_plt_color,
                                                     fill=True))
                        if id == 0:
                            for ind in range(agent.policy.FORCES_N):
                                alpha = 1 - ind * agent.policy.dt / agent.policy.FORCES_N
            ...
            alpha = 0.7
            c = rgba2rgb(plt_color + [float(alpha)])
            ax.add_patch(plt.Circle(agent.global_state_history[agent.step_num - 1, 1:3],
                                    radius=agent.radius, fc=c, ec=plt_color))
            # y_text_offset = 0.1
            # ax.text(agent.global_state_history[ind, 1] - 0.15,
            #         agent.global_state_history[ind, 2] + y_text_offset,
...
def draw_agent_ig(agent, i, ax):
    ...
    if hasattr(agent.ig_model, "expert_policy"):
        ...
        c = rgba2rgb(plt_color + [float(alpha)])
        heading = 0.0
        ax.add_patch(Wedge(center=pose[0:2], r=.2, theta1=(heading - fov / 2), theta2=(heading + fov / 2), fc=c, ec=c,
                           fill=True))
...
def plot_perturbed_observation(agents, ax, perturbed_info):
    ...
    for i, agent in enumerate(agents):
        ...
        eps_lower_left_before_rotation = np.dot(np.array(
